...
AS
(
select product_name,year, ANY_VALUE(price),SUM(sales_value) as yearly_sales_value, SUM(sales_volume) as yearly_sales_amt, SUM(transaction_cnt) as transaction_cnt
from basis
...
as
(
select year,product_name, NTILE(5) OVER (PARTITION BY year ORDER BY sales_value desc)
...
as
(
select year, bucket_nr,MIN(sales_value) as minimum_val, MAX(sales_value) as maximum_val, MIN(price) as least_expensive_product, MAX(price) as most_expensive_product
...
as
(
select distinct year,bucket_nr, ROUND(AVG(SUM(transaction_cnt)) OVER (PARTITION BY year,bucket_nr),3)
...
as
(
select year, bucket_nr, most_expensive_sales_volume-avg_sales_volume roznica_najdrozszy_srednia,least_expensive_sales_volume-avg_sales_volume roznica_najtanszy_srednia
from
(
select year, bucket_nr, ROUND(AVG(sales_volume)) avg_sales_volume, MAX(sales_volume) KEEP (DENSE_RANK FIRST ORDER BY price desc) most_expensive_sales_volume, MAX(sales_volume) KEEP (DENSE_RANK FIRST ORDER BY price asc) least_expensive_sales_volume
from yearly_product_sales JOIN product_bucket_assignment USING(product_name,year)
group by year, bucket_nr
))
