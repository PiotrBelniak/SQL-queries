select p.name as product_name, c.month_id,APPROX_SUM(s.quantity*p.price) approximation_of_sum
...
select prod_typ,month_id, monthly_sales, ROUND(percentage_of_sales,4)*100
from
(
select p.prod_typ, c.month_id, SUM(s.quantity*p.price) monthly_sales,MAX(SUM(s.quantity*p.price)) OVER (partition by prod_typ) as best_month, ratio_to_report(SUM(s.quantity*p.price)) OVER (partition by month_id) as percentage_of_sales
from sales s,product p, calendar c
