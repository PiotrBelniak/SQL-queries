CREATE OR REPLACE FORCE NONEDITIONABLE VIEW PROD_TIME_CUBE (HIERARCHICAL_TIME, YEAR, QUARTER_ID, MONTH_ID, DATA, SUPERTYP, PROD_TYP, NAME, GID, GID_P, GID_T, REVENUE, SOLD_PIECES, REVENUE_CNT, NUM_OF_TRANS, CNT) AS 
  SELECT (CASE 
        ...
        ,c.year, c.quarter_id, c.month_id, c.data, p.supertyp,p.prod_typ,p.name
        ...
        ,SUM(s.quantity*p.price) as revenue, SUM(s.quantity) as sold_pieces, COUNT(s.quantity*p.price) as revenue_cnt,COUNT(s.quantity) as num_of_trans, COUNT(*) cnt
...
CREATE OR REPLACE FORCE NONEDITIONABLE VIEW COMPLETE_TIMELINE (HIERARCHICAL_TIME, YEAR, QUARTER_ID, QUARTER_NUM, MONTH_ID, MONTH_NUM, DATA, DAY_NUM, GID_T) AS 
  SELECT (CASE 
        ...
        ,year, quarter_id, SUBSTR(quarter_id,5,1) quarter_num, month_id, SUBSTR(month_id,5,2) month_num, data , data - TRUNC(data, 'YEAR')+1 day_num
...
select name, hierarchical_time, year,quarter_id,month_id, data,sales, sales_prior_period,sales_yty, gid
from
(
select name, hierarchical_time, year,quarter_id,month_id, data,gid,sales, LAG(sales,1) OVER (PARTITION BY gid_p,supertyp,prod_typ,name, gid_t ORDER BY year,quarter_id,month_id,data) sales_prior_period,
LAG(sales,1) OVER (PARTITION BY gid_p,supertyp,prod_typ,name, gid_t,quarter_num,month_num,day_num ORDER BY year) sales_yty
from
(
select t.hierarchical_time, t.year,t.quarter_id,t.quarter_num,t.month_id,t.month_num,t.data,t.day_num,t.gid_t,c.supertyp,c.prod_typ,c.name,c.gid,c.gid_p,NVL(c.revenue,0) sales
from
prod_time_cube c
